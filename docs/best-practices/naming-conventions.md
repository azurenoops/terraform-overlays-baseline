...
resource "azurerm_nat_gateway" "this" {
  ...
  depends_on = [azurerm_internet_gateway.this]
...
```hcl
resource "azurerm_nat_gateway" "that" {    # Best
  count = var.create_public_subnets ? 1 : 0
}

resource "azurerm_nat_gateway" "this" {    # Good
...
2. Good structure for the name of output looks like `{name}_{type}_{attribute}` , where:
   1. `{name}` is a resource or data source name
      * `{name}` for `data "azurerm_subnet" "private"` is `private`
      * `{name}` for `resource "azurerm_vpc_endpoint_policy" "test"` is `test`
   2. `{type}` is a resource or data source type without a provider prefix
      * `{type}` for `data "azurerm_subnet" "private"` is `subnet`
      * `{type}` for `resource "azurerm_vpc_endpoint_policy" "test"` is `vpc_endpoint_policy`
...
output "security_group_id" {
  ...
  value       = try(azurerm_security_group.this[0].id, azurerm_security_group.name_prefix[0].id, "")
}
```
...
output "this_security_group_id" {
  description = "The ID of the security group"
  value       = element(concat(coalescelist(azurerm_security_group.this.*.id, azurerm_security_group.web.*.id), [""]), 0)
}
```
...
output "rds_cluster_instance_endpoints" {
  ...
  value       = azurerm_rds_cluster_instance.this.*.endpoint
}
```
